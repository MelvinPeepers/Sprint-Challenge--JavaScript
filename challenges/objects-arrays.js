...
const contactInfo = [];
for (let i = 0; i < graduates.length; i++) {
  let nameEmailAddress = [];
  nameEmailAddress.push(graduates[i]["first_name"] + " " + graduates[i].email);

  contactInfo.push(nameEmailAddress.toString());
}
...
for (let i = 0; i < graduates.length; i++) {
  let findUni = 'Uni';
  let uniName = "";
  uniName = graduates[i].university;

  if (uniName.includes(findUni) === true) {
...
let lowerCase = [];
lowerCase = zooAnimals.map(names => {
...
largerPopulation = zooAnimals.filter(num => {
